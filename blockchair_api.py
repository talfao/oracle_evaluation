...
def main():
    ...
    i = 0
    with open("statistics/EthereumAddressWithTXCount.csv", "a") as address_file:
        address_file.write(f"Address, TX count, Call count\n")
    with open("statistics/LatestRoundDataEthereum.csv", "r") as f:
        for line in f.readlines():
            ...
            # print(address)
            response = requests.get(f"https://api.blockchair.com/ethereum/dashboards/address/{address}?limit=1&offset=0&key={api_key}")
            ...
            if type_of_address == "contract":
                with open("statistics/EthereumAddressWithTXCount.csv", "a") as address_file:
                    address_file.write(f"{address},{info.get('transaction_count')},{info.get('call_count')}\n")
